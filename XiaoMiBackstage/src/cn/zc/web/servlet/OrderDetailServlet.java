...
import cn.zc.service.impl.OrderDetailServiceImpl;
...
//    获取订单页面
    public String getOrderDetailView(HttpServletRequest request, HttpServletResponse response) throws Exception {
        request.setCharacterEncoding("utf-8");
        response.setContentType("text/html;charset=utf-8");

        OrderDetailService orderDetailService = new OrderDetailServiceImpl();
        List<OrderDetail> lists = null;
        ...
        // 获取总数据条数
        int result = orderDetailService.getCount();

        if (sers != null){
            // 如果不为空,就模糊查询订单
            lists= orderDetailService.findallByName(sers);
            jsonObject.put("count",0);
        }else {
            //如果为空,就显示所有订单
            lists = orderDetailService.findAllOrder(limit,page);
